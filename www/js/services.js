  ...
  var init = function(){
    ...
    var calc = function(game, count) {
      ...
      for (var i = 0; i < count;i++){
        ...
        if (allNumbers.length < game.normalNumbersCount){
          allNumbers = game.normalNumbers.slice();
        }
        for (var j = 0; j < game.normalNumbersCount; j++){
          var index=Math.floor(Math.random() * allNumbers.length);
          lotto.values.push(allNumbers[index]);
          allNumbers.splice(index,1);
        }

        if (allExtras.length < game.specialNumbersCount){
          allExtras = game.specialNumbers.slice();
        }
        for (var j = 0; j < game.specialNumbersCount; j++){
    ...
    games = [
              {
                ...
                calcPredict : function(count){
                  return calc(this, count);
                },
                result : {},
                normal_number : 'gewinnzahlen',
                special_number : 'superzahl'
               },
               {
                  id  : 1,
                  name : 'Euro Jackpot',
                  nick : 'ej',
                  logo : 'img/logo-ej.svg',
                  apis : {
    ...
    actions = {
      ...
      get : function(gameId) {
        for (var key in games){
          ...
          if (games[key].id == gameId || games[key].id ==  parseInt(gameId) ){
            return games[key];
          }
        }
        return this.get(0);
